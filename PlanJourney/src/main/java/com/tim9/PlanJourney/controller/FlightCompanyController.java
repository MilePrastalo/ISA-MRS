...
public class FlightCompanyController {
	...
	public @ResponseBody FlightCompany getFlightCompanyProfile() throws Exception {		
		...
		if (flightC == null) {
			...
			Flight flight = new Flight(new Date(),new Date(), 3,6000,startDestination,endDestination,new HashSet<Ticket>(),new HashSet<Seat>(),1000,1000,1000);
			flightService.save(flight);
			Set<Flight> flights = new HashSet<>();
			flights.add(flight);
			flightC = new FlightCompany("Avio","address","description",0, new HashSet<FlightAdmin>(), new HashSet<Destination>(),flights, new HashSet<Ticket>() );
	...
	@CrossOrigin()
	public @ResponseBody FlightCompany updateFlightCompanyProfile(@RequestBody FlightCompany updatedFC) throws Exception {
	...
	public @ResponseBody Destination addDestination(Destination destInfo) throws Exception {
		...
		Destination newDestination = new Destination(destInfo.getName(),destInfo.getDescription(),destInfo.getCoordinates());
	...
	@RequestMapping(
			value = "/api/getFlights",
			method = RequestMethod.GET,
			produces = MediaType.APPLICATION_JSON_VALUE)
	@CrossOrigin()
	public @ResponseBody ArrayList<Flight> getFlights() throws Exception {		
		//treba mi prvo ulogovan korisnik da je flight admin i njegova avio kompanija i onda da uzmem njene letove
		System.out.println("POZVAN SAM");
		FlightCompany flightC = FCservice.findByName("Avio");
		if (flightC == null) {
			flightC = new FlightCompany("Avio","address","description",0, new HashSet<FlightAdmin>(), new HashSet<Destination>(),new HashSet<Flight>(), new HashSet<Ticket>() );
			FCservice.save(flightC);
		}	
	...
	public @ResponseBody ArrayList<Destination> getDestinations() throws Exception {
		...
		//treba mi prvo ulogovan korisnik da je flight admin i njegova avio kompanija i onda da uzmem njene destinacije
		FlightCompany flightC = FCservice.findByName("Avio");
		if (flightC == null) {
			flightC = new FlightCompany("Avio","address","description",0, new HashSet<FlightAdmin>(), new HashSet<Destination>(),new HashSet<Flight>(), new HashSet<Ticket>() );
			FCservice.save(flightC);
